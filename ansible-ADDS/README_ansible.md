...
## Steps to adding a Linux VM (CentOS) to a AD DS win 2019 server

***Used Bridged network adapter for this set up***

- Make sure Your AD domain (pumej.com) is reachable from your CentOS VM:

```bash
ping pumej.com
ping pumejlab.pumej.com                             | Confirm line of sight to server
```

- Make sure DNS on your CentOS points to the AD Domain Controller’s IP, not a public DNS or your BIND test server:

```bash
sudo nmcli device modify eth0 ipv4.dns 192.168.1.103
sudo systemctl restart NetworkManager

OR just update the /etc/resolv.conf file 
...
- Install required packages for Centos to support AD DS domain join 
...
What these do:
    ...
    realmd – handles AD discovery and joining
    sssd – authenticates users from AD
    adcli – communicates with AD for join operations
    oddjob-mkhomedir – auto-creates home directories for AD users

- Discover your AD domain using below command: 

```bash
sudo realm discover pumej.com                               | Used to test/discover domain, sample output is shown below 

pumej.com
    type: kerberos
    realm-name: PUMEJ.COM
    domain-name: pumej.com
    ...
    login-formats: %U@pumej.com
    login-policy: allow-realm-logins
```
- Join the domain using the below command: 

```bash
sudo realm join --user=Administrator pumej.com                                  | Would prompt for the user password
sudo realm permit --all                                                         | Used to permit all AD users 
sudo realm permit "Domain Admins"                                               | Used to allow a specific group 
sudo authselect select sssd with-mkhomedir --force                              | Used to auto create home directories once user logs in.
su - Administrator@pumej.com                                                    | Used to switch to a user. 
sudo realm leave pumej.com
```
- If you ever want to unjoin: leaving the domain use below command: 

```bash
sudo realm leave pumej.com                                                      | Used to leave the domain 
```